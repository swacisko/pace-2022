...
class NuMVC {
 ...
 private:
  ...
  void init_sol(vector<int> init_vc = {}) {
    ...
    if( init_vc.empty() ) {
        ...
        set<int> zb(ALL(init_vc));
        for (int v = 0; v < v_num; ++v){
//            if(zb.count(v) == 0 ) v_heap.push(v);
            v_heap.push(v);
        }

        sort(ALL(init_vc), [&](auto a, auto b){ return (dscore[a] < dscore[b]); });
        reverse(ALL(init_vc));

        for(int d : init_vc){
            v_heap.erase(v_heap[d]);
            add_init(d, v_heap);
        }

        assert(uncov_stack_fill_pointer == 0);

        c_size = init_vc.size();
        clog << "Induced VC solution created" << endl;
    }
