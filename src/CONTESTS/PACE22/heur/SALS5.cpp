...
unsigned long long SALS5::x=123456789, SALS5::y=362436069, SALS5::z=521288629;

SALS5::SALS5(VVI V, VI dfvs, Config c) :
...
VI SALS5::localSearch(double T0, double alpha, int maxMvt, int maxFail, int max_iters) {
    if(cnf.write_logs) clog << "Starting SALS5 local search, T0: " << T0 << endl;
    T = T0;
    nbFail = 0;
...
//                if (total_backgoing_arcs == 0) { // this was the case in SALS3 ors SALS4, not here
...
void SALS5::insertIntoOrder(int v, int pos) {
    dfvs_size--;
...
void SALS5::removeFromOrder(int v) {
    dfvs_size++;
    assert(in_order[v] != -1);
...
void SALS5::applyMove(int v, int pos) {
//    int to_remove = getNodeToRemoveFromOrder();
    ...
    StandardUtils::makeUnique(conflicts);

    for(int d : conflicts) removeFromOrder(d);
    insertIntoOrder(v, pos);
}

void SALS5::updateNode(int v) {
    delta[v].clear();
    ...
    /**
     * If true, then at most 2 best insertion points will be kept. This seems to work better and is a bit faster
     */
    constexpr bool keep_at_most_two_best_insertion_points = true;
    if(keep_at_most_two_best_insertion_points){
        auto it1 = min_element( ALL(delta[v]), [&](auto &a, auto &b){
            if( a.cnt != b.cnt ) return a.cnt < b.cnt;
            else return in_order[a.u] < in_order[b.u];
        } );

        auto it2 = min_element( ALL(delta[v]), [&](auto &a, auto &b){
            if( a.cnt != b.cnt ) return a.cnt < b.cnt;
            else return in_order[a.u] > in_order[b.u];
        } );

        int ind1 = it1 - delta[v].begin();
        int ind2 = it2 - delta[v].begin();

        vector<INS_PT> new_delta; new_delta.reserve(2);
        if( ind1 != ind2 ){
            new_delta.push_back(delta[v][ind1]);
            new_delta.push_back(delta[v][ind2]);
        }else{
            if( delta[v].size() >= 2 ){
                nth_element(delta[v].begin(), delta[v].begin() + 1, delta[v].end(),
                                                   [](auto a, auto b){ return a.cnt < b.cnt; });
            }
            for( int i=0; i<min(2, (int)delta[v].size()); i++ ) new_delta.push_back(delta[v][i]);
        }

        swap(delta[v],new_delta);
    }
...
unsigned long long SALS5::xorshf96() {          //period 2^96-1
    unsigned long t;
    x ^= x << 16;
    x ^= x >> 5;
    x ^= x << 1;
...
double SALS5::findInitialTemperature(VVI V, VI& dfvs, Config cnf, int max_mvt) {
//    double T0 = 0.7; // original
    double T0 = 0.25; // #TEST
    ...
    while( T0 >= 0.085 ){
        ...
        sals5.cnf.write_logs = false;

        auto temp = sals5.localSearch( T0, alpha, max_mvt, 1, 1 );
...
//void SALS5::moveRandomNodeToOrder() {
